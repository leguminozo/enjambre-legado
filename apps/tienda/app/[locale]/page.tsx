import { getSiteContentBatch, type SiteSectionItem } from '@/lib/cms';
import { getEcosystemMetrics, type EcosystemMetrics } from '@/lib/shop/ecosystem-metrics';
import { listVisibleProducts, type ShopProduct } from '@/lib/shop/products';
import { TiendaLandingView } from '@/app/landing-view';
import { getTranslations } from 'next-intl/server';

type ColeccionItem = { kicker: string; title: string; desc: string; href: string };
type FooterBranding = { tagline: string; email: string };
type FooterLink = { label: string; href: string };
type MediaItem = { type: 'image' | 'video'; src: string; alt?: string };

function extractContent<T extends Record<string, unknown>>(items: SiteSectionItem[]): T[] {
  return items.map((item) => item.content as T);
}

export const revalidate = 300;

export default async function TiendaPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'common' });
  const tMedia = await getTranslations({ locale, namespace: 'media' });
  
  const [cmsBatch, ecosystemMetrics, products] = await Promise.all([
    getSiteContentBatch(['colecciones', 'footer_branding', 'footer_nav', 'footer_legal']),
    getEcosystemMetrics(),
    listVisibleProducts(),
  ]);

  const coleccionesData = cmsBatch.colecciones ?? [];
  const footerBrandingData = cmsBatch.footer_branding ?? [];
  const footerNavData = cmsBatch.footer_nav ?? [];
  const footerLegalData = cmsBatch.footer_legal ?? [];

  const colecciones = coleccionesData.length > 0
    ? extractContent<ColeccionItem>(coleccionesData)
    : [
        { kicker: t('collections.sachets'), title: t('collections.nectarDrops'), desc: t('collections.nectarDropsDesc'), href: '/catalogo' },
        { kicker: t('collections.mediumJars'), title: t('collections.hiveTreasures'), desc: t('collections.hiveTreasuresDesc'), href: '/catalogo' },
        { kicker: t('collections.largeJars'), title: t('collections.forestReserves'), desc: t('collections.forestReservesDesc'), href: '/catalogo' },
        { kicker: t('collections.virginHoney'), title: t('collections.hiveHoneycomb'), desc: t('collections.hiveHoneycombDesc'), href: '/catalogo' },
        { kicker: t('collections.sachetBoxes'), title: t('collections.swarmChests'), desc: t('collections.swarmChestsDesc'), href: '/catalogo' },
        { kicker: t('collections.subscriptions'), title: t('collections.forestLegacy'), desc: t('collections.forestLegacyDesc'), href: '/catalogo' },
      ];

  const mediaItems: MediaItem[] = [
    { type: 'image', src: '/assets/editorial/immersion.png', alt: tMedia('immersionAlt') },
    { type: 'image', src: '/assets/editorial/sachets.png', alt: tMedia('sachetsAlt') },
    { type: 'image', src: '/assets/editorial/honey-jar.png', alt: tMedia('honeyJarAlt') },
  ];

  const youtubeVideoId = process.env.NEXT_PUBLIC_YOUTUBE_VIDEO_ID || 'dQw4w9WgXcQ';

  const footerData = {
    branding: (footerBrandingData[0]?.content ?? { tagline: t('footer.tagline'), email: 'hola@obrerayzangano.com' }) as FooterBranding,
    nav: footerNavData.length > 0 ? extractContent<FooterLink>(footerNavData) : [],
    legal: footerLegalData.length > 0 ? extractContent<FooterLink>(footerLegalData) : [],
  };

  return (
    <TiendaLandingView
      initialColecciones={colecciones}
      products={products}
      mediaItems={mediaItems}
      youtubeVideoId={youtubeVideoId}
      footerData={footerData}
      ecosystemMetrics={ecosystemMetrics}
      locale={locale}
    />
  );
}