...
export type PollFacturaEmitidaResult =
  | {
      ok: true;
...
export async function pollFacturaEmitidaSii(
  supabase: SupabaseClient,
  empresaId: string,
  facturaId: string,
): Promise<PollFacturaEmitidaResult> {
  const { data: factura } = await supabase
    .from('facturas_emitidas')
    .select('id, track_id, estado_sii')
    .eq('id', facturaId)
    .eq('empresa_id', empresaId)
    .single();

  if (!factura) {
    return { ok: false, code: 'not_found', message: 'Boleta/factura emitida no encontrada' };
  }

  const row = factura as { track_id: string | null; estado_sii: string | null };
  if (!row.track_id) {
    return { ok: false, code: 'no_track_id', message: 'El documento no tiene track_id' };
  }
