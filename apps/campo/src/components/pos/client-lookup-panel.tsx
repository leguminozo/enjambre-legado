    ...
    return (
      <div className="card-glow p-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-stone-500 uppercase tracking-widest">Cliente</span>
        </div>
        <div className="flex gap-2">
          ...
          <button
            ...
            className="text-[10px] uppercase tracking-widest text-stone-400 hover:text-white"
          >
            Buscar
          </button>
        </div>
    ...
    return (
      <div className="card-glow p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-white">{selectedClient.full_name || 'Sin nombre'}</p>
            <p className="text-[10px] text-stone-500">{selectedClient.email || 'Sin email'}</p>
          </div>
          <span className="text-[9px] uppercase tracking-widest text-primary bg-primary/10 px-2 py-0.5 rounded-full">Recurrente</span>
        </div>
        <button onClick={() => setSelectedClient(null)} className="text-stone-500 hover:text-white">
          <X className="w-4 h-4" />
        </button>
      </div>
    );
    ...
    return (
      <div className="card-glow p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-white">Cliente nuevo</p>
            <p className="text-[10px] text-stone-500">Se aplicará comisión base (sin loyalty)</p>
          </div>
          <span className="text-[9px] uppercase tracking-widest text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full">Nuevo</span>
        </div>
        <button onClick={() => setIsNewClient(false)} className="text-stone-500 hover:text-white">
          <X className="w-4 h-4" />
        </button>
      </div>
    );
  ...
  return (
    <div className="card-glow p-4 space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-xs text-stone-500 uppercase tracking-widest">Buscar cliente</span>
        <button onClick={() => { setExpanded(false); setQuery(''); }} className="text-stone-500 hover:text-white">
          <X className="w-4 h-4" />
        </button>
      </div>
      <div className="relative">
        ...
        <input
          ...
          autoFocus
          className="w-full bg-black/40 border border-stone-800 rounded-lg pl-10 pr-4 py-2.5 text-sm text-white focus:border-primary focus:outline-none transition-colors"
        />
        {searching && (
      ...
      {results.length > 0 && (
        <div className="space-y-1 max-h-40 overflow-y-auto">
          {results.map((r) => (
            ...
            >
              <div>
                <p className="text-sm font-medium text-white">{r.full_name || 'Sin nombre'}</p>
                <p className="text-[10px] text-stone-500">{r.email || r.phone || 'Sin contacto'}</p>
              </div>
