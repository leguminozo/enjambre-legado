    ...
    return (
        <div>
                        ...
                        <div style={{ height: 400 }}>
                            <ResponsiveContainer width="100%" height="100%">
                                {expandedChart === 'production' ? (
                                    ...
                                    <AreaChart data={cashFlowData}><CartesianGrid strokeDasharray="3 3" stroke="rgba(10,61,47,0.08)" /><XAxis dataKey="month" tick={{ fontSize: 12 }} stroke="#8A9AAF" /><YAxis tick={{ fontSize: 12 }} stroke="#8A9AAF" /><Tooltip contentStyle={{ borderRadius: 8, border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', fontFamily: 'Inter', fontSize: '0.82rem' }} /><Area type="monotone" dataKey="income" stroke="#2ECC71" fill="rgba(46,204,113,0.12)" strokeWidth={2} /><Area type="monotone" dataKey="expenses" stroke="#E74C3C" fill="rgba(231,76,60,0.08)" strokeWidth={2} /></AreaChart>
                                )}
                            </ResponsiveContainer>
                        </div>
                    </div>
            ...
            <div className="dashboard-grid dashboard-grid-2-1" style={{ marginTop: 'var(--space-lg)' }}>
                <div className="card animate-in delay-3"><div className="section-header"><div><div className="section-title">Flujo de Caja</div><div className="section-subtitle">Ingresos vs Egresos (miles CLP)</div></div><button className="btn btn-ghost btn-sm" onClick={() => setExpandedChart('cashflow')}><Expand size={14} /></button></div><div style={{ height: 220 }}><ResponsiveContainer width="100%" height="100%"><AreaChart data={cashFlowData}><CartesianGrid strokeDasharray="3 3" stroke="rgba(10,61,47,0.08)" /><XAxis dataKey="month" tick={{ fontSize: 12 }} stroke="#8A9AAF" /><YAxis tick={{ fontSize: 12 }} stroke="#8A9AAF" /><Tooltip contentStyle={{ borderRadius: 8, border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', fontFamily: 'Inter', fontSize: '0.82rem' }} /><Area type="monotone" dataKey="income" stroke="#2ECC71" fill="rgba(46,204,113,0.12)" strokeWidth={2} /><Area type="monotone" dataKey="expenses" stroke="#E74C3C" fill="rgba(231,76,60,0.08)" strokeWidth={2} /></AreaChart></ResponsiveContainer></div></div>
