-- Seeding default menu settings (idempotent)
INSERT INTO site_content (section_key, item_order, content)
SELECT 'menu_settings', 0, '{"menu_format": "horizontal", "height_desktop": 80, "height_mobile": 64, "letter_spacing": "0.2em", "button_gap": "2.5rem"}'::jsonb
WHERE NOT EXISTS (SELECT 1 FROM site_content WHERE section_key = 'menu_settings');

-- Seeding default menu links (idempotent)
INSERT INTO site_content (section_key, item_order, content)
SELECT v.section_key, v.item_order, v.content::jsonb
FROM (
  VALUES
