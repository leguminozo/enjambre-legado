...
export const stripeParser: ReceiptParser = {
  id: "stripe",

  detect(text: string): boolean {
    const lower = text.toLowerCase();
    return lower.includes("stripe") && (lower.includes("fee") || lower.includes("payment") || lower.includes("charge") || lower.includes("payout"));
  },

  parse(text: string, config: ProveedorConfig, tasaCambio: number): GastoExtranjeroResult | null {
    const totalMatch = text.match(/(?:Total|Amount|Fee|Charge|Net)\s*:?\s*(?:USD|US\$|\$)\s*([\d,]+\.?\d*)/i)
      ?? text.match(/(?:Total|Amount|Fee|Charge|Net)\s*:?\s*([\d,]+\.\d{2})/i)
      ?? text.match(/\$\s*([\d,]+\.\d{2})/);
    ...
    const txnMatch = text.match(/(?:Transaction|Charge|Payment)\s*(?:#|ID)\s*:?\s*([\w-]+)/i)
      ?? text.match(/(?:ch|py|txn)_[\w]+/i);
    ...
    return {
      ...
      numeroDocumento: txnMatch ? (typeof txnMatch === "string" ? txnMatch : txnMatch[1]) : "",
      concepto: "Comision por procesamiento de pagos Stripe",
      detalle: `Stripe Fee${txnMatch ? ` - ${typeof txnMatch === "string" ? txnMatch : txnMatch[1]}` : ""}`,
    };
  },
};
