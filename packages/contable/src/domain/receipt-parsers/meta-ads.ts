...
export const metaAdsParser: ReceiptParser = {
  id: "meta-ads",

  detect(text: string): boolean {
    const lower = text.toLowerCase();
    return lower.includes("meta ads") || lower.includes("facebook ads") || lower.includes("instagram ads") || lower.includes("meta for business") || (lower.includes("meta") && (lower.includes("billing") || lower.includes("invoice") || lower.includes("payment")));
  },

  parse(text: string, config: ProveedorConfig, tasaCambio: number): GastoExtranjeroResult | null {
    const totalMatch = text.match(/(?:Total|Amount|Amount due|Total due|Total charge)\s*:?\s*(?:USD|US\$|\$)\s*([\d,]+\.?\d*)/i)
      ?? text.match(/(?:Total|Amount|Amount due|Total due|Total charge)\s*:?\s*([\d,]+\.\d{2})/i)
      ?? text.match(/\$\s*([\d,]+\.\d{2})/);
    ...
    const invoiceMatch = text.match(/(?:Invoice|Receipt|Transaction)\s*(?:#|number|no\.?|ID)\s*:?\s*([\w-]+)/i);
    const adAccountMatch = text.match(/(?:Ad|Business)\s*(?:account|ID)\s*:?\s*(\d+)/i);
    ...
    return {
      ...
      numeroDocumento: invoiceMatch?.[1] ?? adAccountMatch?.[1] ?? "",
      concepto: "Servicio de publicidad digital Meta Ads",
      detalle: `Meta Ads${adAccountMatch ? ` - Cuenta ${adAccountMatch[1]}` : ""}`,
    };
  },
};
