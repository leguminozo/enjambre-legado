...
export const shopifyParser: ReceiptParser = {
  id: "shopify",

  detect(text: string): boolean {
    const lower = text.toLowerCase();
    return lower.includes("shopify") && (lower.includes("billing") || lower.includes("invoice") || lower.includes("payment") || lower.includes("subscription"));
  },

  parse(text: string, config: ProveedorConfig, tasaCambio: number): GastoExtranjeroResult | null {
    const totalMatch = text.match(/(?:Total|Amount|Amount due|Total due|Total charge|Subscription)\s*:?\s*(?:USD|US\$|\$)\s*([\d,]+\.?\d*)/i)
      ?? text.match(/(?:Total|Amount|Amount due|Total due|Total charge|Subscription)\s*:?\s*([\d,]+\.\d{2})/i)
      ?? text.match(/\$\s*([\d,]+\.\d{2})/);
    ...
    const invoiceMatch = text.match(/(?:Invoice|Receipt|Bill)\s*(?:#|number|no\.?|ID)\s*:?\s*([\w-]+)/i);
    const planMatch = text.match(/(?:Plan|Subscription)\s*:?\s*(.+?)(?:\n|$)/i);
    ...
    return {
      ...
      concepto: "Servicio de plataforma e-commerce Shopify",
      detalle: `Shopify${planMatch ? ` - ${planMatch[1].trim()}` : ""}`,
    };
  },
};
