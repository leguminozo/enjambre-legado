...
export const awsParser: ReceiptParser = {
  id: "aws",

  detect(text: string): boolean {
    const lower = text.toLowerCase();
    return lower.includes("amazon web services") || lower.includes("aws billing") || lower.includes("aws ") || lower.includes("amazon cloudfront");
  },

  parse(text: string, config: ProveedorConfig, tasaCambio: number): GastoExtranjeroResult | null {
    const totalMatch = text.match(/(?:Total|Amount|Amount due|Total due|Total charges|Charges)\s*:?\s*(?:USD|US\$|\$)\s*([\d,]+\.?\d*)/i)
      ?? text.match(/(?:Total|Amount|Amount due|Total due|Total charges|Charges)\s*:?\s*([\d,]+\.\d{2})/i)
      ?? text.match(/\$\s*([\d,]+\.\d{2})/);
    ...
    const invoiceMatch = text.match(/(?:Invoice|Statement)\s*(?:#|number|no\.?|ID)\s*:?\s*([\w-]+)/i);
    const accountIdMatch = text.match(/(?:Account|Payer)\s*ID\s*:?\s*(\d+-\d+-\d+)/i);
    ...
    return {
      ...
      concepto: "Servicio de computacion en la nube AWS",
      detalle: `AWS${accountIdMatch ? ` - Cuenta ${accountIdMatch[1]}` : ""}`,
    };
  },
};
